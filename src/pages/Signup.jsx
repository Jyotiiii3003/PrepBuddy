...
export default function Signup() {
  ...
  if (success) {
    return (
      <div style={{ minHeight: "100vh", background: COLORS.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans','Segoe UI',sans-serif", padding: "2rem" }}>
        <div style={{ textAlign: "center", maxWidth: 480 }}>
          ...
          <h2 style={{ fontSize: 28, fontWeight: 800, color: COLORS.secondary, marginBottom: 12 }}>You are in!</h2>
          ...
          <button
            ...
            style={{ background: COLORS.primary, border: "none", color: "#fff", padding: "13px 32px", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer", boxShadow: `0 6px 20px ${COLORS.primary}44` }}>
            Go to Login →
          </button>
        </div>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: COLORS.bg, display: "flex", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 2rem" }}>
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
            {[1, 2].map((s) => (
              <div key={s} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: step >= s ? COLORS.primary : COLORS.border, color: step >= s ? "#fff" : COLORS.textMuted, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, transition: "background 0.3s" }}>
                  {step > s ? "✓" : s}
                </div>
                <span style={{ fontSize: 12, color: step >= s ? COLORS.primary : COLORS.textMuted, fontWeight: step >= s ? 600 : 400 }}>
          ...
          <p style={{ fontSize: 14, color: COLORS.textMuted, marginBottom: 32 }}>
            {step === 1 ? (
              <>Already have an account?{" "}
                <span onClick={() => navigate("/login")} style={{ color: COLORS.primary, fontWeight: 600, cursor: "pointer", textDecoration: "underline" }}>Sign in</span>
              </>
          ...
          {step === 1 ? (
            <>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: COLORS.secondary, marginBottom: 8 }}>Full name</label>
                <input
                  type="text"
                  name="name"
                  value={form.name}
              ...
              {/* Email */}
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: COLORS.secondary, marginBottom: 8 }}>Email address</label>
                <input
                  type="email"
                  name="email"
                  value={form.email}
              ...
              <div style={{ marginBottom: 28 }}>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: COLORS.secondary, marginBottom: 8 }}>Password</label>
              ...
              <button
                ...
                style={{ width: "100%", padding: "14px", borderRadius: 12, background: COLORS.primary, border: "none", color: "#fff", fontSize: 16, fontWeight: 700, cursor: "pointer", transition: "transform 0.2s", boxShadow: `0 6px 20px ${COLORS.primary}44` }}>
                Continue →
              </button>
            </>
          ) : (
            <>
              {/* College */}
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: COLORS.secondary, marginBottom: 8 }}>Your college</label>
                <select
                  name="college"
                  value={form.college}
                  onChange={handleChange}
                  onFocus={e => e.target.style.borderColor = COLORS.primary}
                  onBlur={e => e.target.style.borderColor = COLORS.border}
                  style={{ width: "100%", padding: "12px 16px", borderRadius: 10, border: `1.5px solid ${COLORS.border}`, background: "#fff", fontSize: 15, color: form.college ? COLORS.text : COLORS.textMuted, outline: "none", boxSizing: "border-box", transition: "border-color 0.2s", cursor: "pointer" }}>
              ...
              <div style={{ marginBottom: 28 }}>
                ...
                <select
                  ...
                  onFocus={e => e.target.style.borderColor = COLORS.primary}
                  onBlur={e => e.target.style.borderColor = COLORS.border}
                  style={{ width: "100%", padding: "12px 16px", borderRadius: 10, border: `1.5px solid ${COLORS.border}`, background: "#fff", fontSize: 15, color: form.target ? COLORS.text : COLORS.textMuted, outline: "none", boxSizing: "border-box", transition: "border-color 0.2s", cursor: "pointer" }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button
                  ...
                  style={{ flex: 1, padding: "14px", borderRadius: 12, background: "transparent", border: `1.5px solid ${COLORS.borderDark}`, color: COLORS.secondary, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>
                  ← Back
                </button>
                <button
          ...
          <p style={{ fontSize: 12, color: COLORS.textMuted, textAlign: "center", marginTop: 24, lineHeight: 1.6 }}>
